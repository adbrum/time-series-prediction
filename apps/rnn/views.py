...
# plot_acf(ts, periodicity, max_lag=240)
...
year_series = datetime_attribute_timeseries(
    pd.date_range(start=series.start_time(), freq=ts.freq_str, periods=1000),
    attribute="year",
    one_hot=False,
)
...
def fit_it(model, train, val, flavor):
    ...
    print(
        "training of the {0} RNN has completed:".format(flavor), f"{res_time:.2f} sec"
    )
...
def plot_fitted(pred, act, flavor):
    ...
    plt.title(
        "RNN: {0} flavor".format(flavor) + " | MAPE: {:.2f}%".format(mape(pred, act))
    )
    plt.legend()
